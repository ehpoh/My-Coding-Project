...
public class ProcessFineStaff extends JFrame{
	...
	public ProcessFineStaff(List<ProcessFineDetail> processFine){
	...
	class ButtonEditor extends DefaultCellEditor {
		...
		public ButtonEditor(JCheckBox checkBox, List<ProcessFineDetail> processFineList) {
	        ...
	        button.addActionListener(e -> {
	            ...
	            JButton close = new JButton("Complete Payment");
	            close.addActionListener(ev -> {
				    // Show message before closing
				    JOptionPane.showMessageDialog(dialog, "Payment completed successfully.", "Confirmation", JOptionPane.INFORMATION_MESSAGE);
				    // Update the table: set Pay Status to "Complete", and change payDate to today date
				    table.setValueAt("Complete", table.getSelectedRow(), 7); // column 7 = Pay Status
				    LocalDate today = LocalDate.now();  // Get current date and time
    				String formattedDate = today.format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));
					table.setValueAt(formattedDate, table.getSelectedRow(), 5); // Column 5 = Pay Date
				    // Optionally disable the button visually (not strictly needed since renderer checks status)
				    button.setEnabled(false);
				    //save the update data
				    String processID = table.getValueAt(row, 0).toString(); // assuming ID is unique
				    ...
				    saveProcessFineToFile("processFine.txt", processFineList);  // <-- Make sure processFineList is accessible
				    dialog.dispose();
				});
